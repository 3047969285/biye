...
@Slf4j
@Service
public class DL698TcpCollectServiceImpl implements DL698TcpCollectService {
    // 默认超时
    ...
    @Override
    public Map<String, String> collectOadAndSave(String ip, Integer port, String meterAddress, Integer timeoutMs, Integer clientAddress, Map<String, String> oadMapping) {
        ...
        protocol.setResponseListener(new ResponseListener() {
            ...
            public void onError(String errorMessage) {
                log.error("DL698 TCP监听错误: {}", errorMessage);
            }

            @Override
            public void onPortOpened(String portName) {
                log.info("DL698 TCP通道已打开: {}", portName);
            }

            @Override
            public void onPortClosed(String portName) {
                log.info("DL698 TCP通道已关闭: {}", portName);
            }
        ...
        try {
            if (!protocol.connectTcp(ip, finalPort, finalTimeout)) {
            ...
            Map<String, OadConfig> commonOadMap = Dl698OadConfigSupport.loadOadMap(oadMapping);
            for (Map.Entry<String, OadConfig> entry : commonOadMap.entrySet()) {
                ...
                if (frame == null) {
                    log.error("TCP查询失败: pointCode={}, di={}", pointCode);
            ...
            log.error("DL698 批量采集失败: {}", e.getMessage());
...
//            }
    ...
    private byte[] queryWithRetry(DL698Protocol protocol,
        ...
        for (int i = 0; i <= QUERY_RETRY_TIMES; i++) {
            responseQueue.clear();
            protocol.sendReadTcp(serverBytes, client, oad, logicAddress, sc);
            ...
            if (i < QUERY_RETRY_TIMES) {
                log.error("DL698 点位{} 第{}次超时，准备重试", pointCode, i + 1);
                Dl698BinarySupport.sleepQuietly(COMMAND_INTERVAL_MS);
            }
        }
        return null;
    ...
    private SessionParam establishSession(DL698Protocol protocol,
        ...
        for (SessionParam candidate : candidates) {
            ...
            log.info("DL698握手成功，client={}, logic={}, sc={}, server={}",
    ...
    private boolean ensureLink(DL698Protocol protocol,
        ...
        for (int i = 0; i <= QUERY_RETRY_TIMES; i++) {
            responseQueue.clear();
            protocol.sendLinkTcp(serverBytes, client, logicAddress, sc);
            ...
            if (i < QUERY_RETRY_TIMES) {
                log.error("DL698 LINK 第{}次失败，准备重试", i + 1);
                Dl698BinarySupport.sleepQuietly(COMMAND_INTERVAL_MS);
            }
        }
        return false;
    ...
    /**
     * 建立 DL698 应用连接：先 CONNECT-Request，再进入 GET 流程
     */
    private boolean ensureConnect(DL698Protocol protocol,
        ...
        for (int i = 0; i <= QUERY_RETRY_TIMES; i++) {
            responseQueue.clear();
            protocol.sendConnectTcp(serverBytes, client, logicAddress, sc);
            byte[] frame = pollUntil(responseQueue, timeoutMs);
            if (frame != null) {
                byte[] apdu = DL698FrameParser.extractApdu(frame);
            ...
            if (i < QUERY_RETRY_TIMES) {
                log.error("DL698 CONNECT 第{}次失败，准备重试", i + 1);
                Dl698BinarySupport.sleepQuietly(COMMAND_INTERVAL_MS);
            }
        }
        return false;
